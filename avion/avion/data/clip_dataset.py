...
class VideoCaptionDatasetCLIP(VideoCaptionDatasetBase):
    def __init__(
        ...
        self.enable_mmts = enable_mmts
        self.distribution_path = distribution_path
        self.use_distribution = use_distribution

        # Build class_id -> frequency count lookup for MMTS
        self.freq_lookup = {}
        if self.enable_mmts:
            if self.use_distribution not in ["verb", "noun", "verb_noun"]:
                raise ValueError("use_distribution must be one of ['verb', 'noun', 'verb_noun']")
            
            if self.distribution_path is None or not os.path.exists(self.distribution_path):
                raise FileNotFoundError(f"MMTS enabled, but invalid distribution_path: {self.distribution_path}")
                
            dist_df = pd.read_csv(self.distribution_path)
            self.freq_lookup = dict(zip(
    ...
    def __getitem__(self, i):
        ...
        if self.enable_mmts:
            if self.dataset not in ["ek100_mir", "ek100_cls"]:
                raise NotImplementedError(f"MMTS logic currently only supports EK100 datasets, got: {self.dataset}")

            *_, verb_class, noun_class = self.samples[i]

            if self.use_distribution == "verb":
                class_key = str(verb_class)
            elif self.use_distribution == "noun":
                class_key = str(noun_class)
            else:
                class_key = f"{verb_class}_{noun_class}"

            return {
                "frames": frames,
                "caption": caption,
                "relevancy": relevancy,
                "cluster_freq": self.freq_lookup.get(class_key, 1),
            }
